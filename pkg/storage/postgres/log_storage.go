...
const (
	...
	selectNonDeletedTreeIDByTypeAndStateSQL = `SELECT tree_id FROM trees WHERE tree_type in ($1,$2) 
                            AND tree_state in ($3,$4) AND (deleted IS NULL OR deleted = false)`
...
func (m *postgresLogStorage) getLeavesByMerkleHashStmt(ctx context.Context, num int,
	orderBySequence bool) (*sql.Stmt, error) {
	if orderBySequence {
...
func (m *postgresLogStorage) beginInternal(ctx context.Context, tree *trillian.Tree) (*logTreeTX, error) {
	...
	if err != nil && !errors.Is(err, storage.ErrTreeNeedsInit) {
		return nil, err
	}
	...
	if errors.Is(err, storage.ErrTreeNeedsInit) {
		...
		ttx.Rollback() // nolint: errcheck, gosec
...
func (m *postgresLogStorage) AddSequencedLeaves(ctx context.Context, tree *trillian.Tree,
	leaves []*trillian.LogLeaf, timestamp time.Time) ([]*trillian.QueuedLogLeaf, error) {
	tx, err := m.beginInternal(ctx, tree)
	if err != nil {
		return nil, err
	}
...
func (m *postgresLogStorage) SnapshotForTree(ctx context.Context,
	tree *trillian.Tree) (storage.ReadOnlyLogTreeTX, error) {
	tx, err := m.beginInternal(ctx, tree)
	if err != nil && !errors.Is(err, storage.ErrTreeNeedsInit) {
		return nil, err
	}
...
func (m *postgresLogStorage) QueueLeaves(ctx context.Context, tree *trillian.Tree,
	leaves []*trillian.LogLeaf, queueTimestamp time.Time) ([]*trillian.QueuedLogLeaf, error) {
	tx, err := m.beginInternal(ctx, tree)
	if err != nil {
		return nil, err
	}
...
func (t *logTreeTX) GetMerkleNodes(ctx context.Context, ids []compact.NodeID) ([]trilliantree.Node, error) {
...
func (t *logTreeTX) DequeueLeaves(ctx context.Context, limit int, cutoffTime time.Time) ([]*trillian.LogLeaf, error) {
	...
	for rows.Next() {
		leaf, dqInfo, err := t.dequeueLeaf(rows) // nolint: govet
		if err != nil {
			return nil, err
		}
...
func (t *logTreeTX) QueueLeaves(ctx context.Context, leaves []*trillian.LogLeaf,
	queueTimestamp time.Time) ([]*trillian.LogLeaf, error) {
	...
	for _, ol := range ordLeaves {
		...
		dupCheckRow, err := t.tx.QueryContext(ctx, insertLeafDataSQL, t.treeID,
			leaf.LeafIdentityHash, leaf.LeafValue, leaf.ExtraData, qTimestamp.UnixNano())
		if err != nil {
		...
		dupCheckRow.Close() // nolint: errcheck, gosec,sqlclosecheck
		...
		queueTimestamp := leaf.QueueTimestamp.AsTime()
		...
		if err != nil {
			return nil, fmt.Errorf("unsequenced: %w -- %v", err, args)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve existing leaves: %w %v", err, toRetrieve)
	}
...
func (t *logTreeTX) AddSequencedLeaves(ctx context.Context, leaves []*trillian.LogLeaf,
	...
	for _, ol := range ordLeaves {
		...
		resultData := true
		for dupCheckRow.Next() {
			dupCheckRow.Scan(&resultData) // nolint: errcheck, gosec
...
func (t *logTreeTX) GetLeavesByHash(ctx context.Context,
	leafHashes [][]byte, orderBySequence bool) ([]*trillian.LogLeaf, error) {
...
func (t *logTreeTX) fetchLatestRoot(ctx context.Context) (*trillian.SignedLogRoot, error) {
	var (
		rootSignatureBytes []byte
		jsonObj            []byte
	)

	t.tx.QueryRowContext( // nolint: gosec
		ctx,
		"select current_tree_data,root_signature from trees where tree_id = $1",
		t.treeID).Scan(&jsonObj, &rootSignatureBytes) // nolint: errcheck
	...
	json.Unmarshal(jsonObj, &logRoot) // nolint: errcheck,gosec

	newRoot, _ := logRoot.MarshalBinary() // nolint: errcheck

	return &trillian.SignedLogRoot{LogRoot: newRoot}, nil
}
...
func (t *logTreeTX) getLeavesByHashInternal(ctx context.Context, leafHashes [][]byte,
	tmpl *sql.Stmt, desc string) ([]*trillian.LogLeaf, error) {
	stx := t.tx.StmtContext(ctx, tmpl)
	defer stx.Close() // nolint: errcheck

	var args []interface{}
	for _, hash := range leafHashes {
		args = append(args, interface{}(hash))
	}
